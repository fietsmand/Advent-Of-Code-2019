var _a = require('./input'), data1 = _a.data1, data2 = _a.data2;
// Central Port
var map = [
    ['.', '.', '.'],
    ['.', '.', '.'],
...
var getOutput = function (startingPoint, index, steps) {
    if (!map[startingPoint.vertical]) {
        if (startingPoint.vertical >= map.length) {
            map.push(map[0].map(function (r) { return '.'; }));
        }
        else {
            map.unshift(map[0].map(function (r) { return '.'; }));
...
var drawLines = function (path) {
    ...
    path.forEach(function (instruction) {
        ...
        if (['R', 'U', 'L', 'D'].includes(direction)) {
            var steps = Number(instruction.slice(1, instruction.length));
            if (direction === 'R') {
...
var findDistance = function () {
    ...
    intersections.map(function (res) {
